...
export default function EachApartment({
  ...
  useEffect(() => {
    const fetcPrivateApartments = async () => {
      ...
      const sliderImagesFromData = data.appartment_images.map((image) => ({
          ...
          <div key={index} className="complexTextsBox2 gapp">
            <div className="seenIdFavouriteAndOthersBox">
              ...
              <div className="favouriteDollarAndShareBox">
                {/* Star favourite box */}
                <button
                  key={eachComplexAllAppartments.id}
                  className="heartButtons"
                  onClick={() =>
                    favorite_apartment_handler(eachComplexAllAppartments)
                  }
                >
                  {favoriteApartment.some(
                    (fav) => fav.id === eachComplexAllAppartments.id
                  ) ? (
